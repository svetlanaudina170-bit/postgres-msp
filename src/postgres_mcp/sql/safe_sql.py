...
from pglast.ast import (
    A_ArrayExpr, A_Const, A_Expr, A_Indices, A_Indirection, A_Star, Alias, BitString,
    Boolean, BooleanTest, BoolExpr, CaseExpr, CaseWhen, ClosePortalStmt, CoalesceExpr,
    CollateClause, ColumnRef, CommonTableExpr, CreateExtensionStmt, DeallocateStmt,
    DeclareCursorStmt, DefElem, ExplainStmt, FetchStmt, Float, FromExpr, FuncCall,
    GroupingFunc, GroupingSet, Integer, JoinExpr, MinMaxExpr, NamedArgExpr, Node,
    NotifyStmt, NullTest, ParamRef, PrepareStmt, RangeFunction, RangeSubselect,
    RangeTableFunc, RangeTableFuncCol, RangeTableSample, RangeVar, RawStmt, ResTarget,
    RowCompareExpr, RowExpr, ScalarArrayOpExpr, SelectStmt, SortBy, SortGroupClause,
    SQLValueFunction, String, SubLink, TableFunc, TableSampleClause, TargetEntry,
    TypeCast, TypeName, VacuumStmt, VariableShowStmt, WithClause, WindowClause, WindowDef, WindowFunc
)
...
class SafeSqlDriver(SqlDriver):
    ...
    ALLOWED_FUNCTIONS: ClassVar[Set[str]] = {
        ...
        "abs", "cbrt", "ceil", "ceiling", "degrees", "div", "erf", "erfc", "exp",
        "factorial", "floor", "gcd", "lcm", "ln", "log", "log10", "min_scale", "mod",
        "pi", "power", "radians", "random", "random_normal", "round", "scale", "setseed",
        "sign", "sqrt", "trim_scale", "trunc", "width_bucket",
        # Тригонометрические функции
        "acos", "acosd", "asin", "asind", "atan", "atand", "atan2", "atan2d", "cos",
        "cosd", "cot", "cotd", "sin", "sind", "tan", "tand",
        # Гиперболические функции
        "sinh", "cosh", "tanh", "asinh", "acosh", "atanh",
        # Функции для работы с массивами
        "array", "array_append", "array_cat", "array_dims", "array_fill", "array_length",
        "array_lower", "array_ndims", "array_position", "array_positions", "array_prepend",
        "array_remove", "array_replace", "array_sample", "array_shuffle", "array_to_string",
        "array_upper", "cardinality", "string_to_array", "trim_array", "unnest", "any",
        # Строковые функции
        "ascii", "bit_length", "btrim", "char_length", "character_length", "chr", "concat",
        "concat_ws", "convert", "convert_from", "convert_to", "decode", "encode", "format",
        "initcap", "left", "length", "lower", "lpad", "ltrim", "md5", "normalize",
        "octet_length", "overlay", "parse_ident", "position", "quote_ident", "quote_literal",
        "quote_nullable", "repeat", "replace", "reverse", "right", "rpad", "rtrim",
        "split_part", "starts_with", "string_to_table", "strpos", "substr", "substring",
        "to_ascii", "to_bin", "to_hex", "to_oct", "translate", "trim", "upper", "unistr",
        "unicode_assigned",
        # Функции для работы с регулярными выражениями
        "regexp_match", "regexp_matches", "regexp_replace", "regexp_split_to_array",
        "regexp_split_to_table", "regexp_substr", "regexp_count", "regexp_instr",
        "regexp_like",
        # Функции приведения типов
        "regclass", "to_char", "to_date", "to_number", "to_timestamp",
        # Информационные функции
        "current_catalog", "current_database", "current_query", "current_role",
        "current_schema", "current_schemas", "current_setting", "current_user",
        "pg_backend_pid", "pg_blocking_pids", "pg_conf_load_time", "pg_current_logfile",
        "pg_jit_available", "pg_safe_snapshot_blocking_pids", "pg_trigger_depth",
        "session_user", "system_user", "user", "version", "unicode_version",
        "icu_unicode_version",
        # Функции информации об объектах базы данных
        "pg_column_size", "pg_column_compression", "pg_column_toast_chunk_id",
        "pg_database_size", "pg_indexes_size", "pg_get_indexdef", "pg_relation_filenode",
        "pg_relation_size", "pg_size_bytes", "pg_size_pretty", "pg_table_size",
        "pg_tablespace_size", "pg_total_relation_size",
        ...
        "has_tablespace_privilege", "has_type_privilege", "pg_has_role",
        "row_security_active",
        # JSON-функции
        "json", "json_array_length", "jsonb_array_length", "json_each", "jsonb_each",
        "json_each_text", "jsonb_each_text", "json_extract_path", "jsonb_extract_path",
        "json_extract_path_text", "jsonb_extract_path_text", "json_object_keys",
        "jsonb_object_keys", "json_array_elements", "jsonb_array_elements",
        "json_array_elements_text", "jsonb_array_elements_text", "json_typeof",
        "jsonb_typeof", "json_strip_nulls", "jsonb_strip_nulls", "jsonb_set",
        "jsonb_set_path", "jsonb_set_lax", "jsonb_pretty", "json_build_array",
        "jsonb_build_array", "json_build_object", "jsonb_build_object", "json_object",
        "jsonb_object", "json_scalar", "json_serialize", "json_populate_record",
        "jsonb_populate_record", "jsonb_populate_record_valid", "json_populate_recordset",
        "jsonb_populate_recordset", "json_to_record", "jsonb_to_record",
        "json_to_recordset", "jsonb_to_recordset", "jsonb_insert", "jsonb_path_exists",
        "jsonb_path_match", "jsonb_path_query", "jsonb_path_query_array",
        "jsonb_path_query_first", "jsonb_path_exists_tz", "jsonb_path_match_tz",
        "jsonb_path_query_tz", "jsonb_path_query_array_tz", "jsonb_path_query_first_tz",
        "jsonbv_typeof", "to_json", "to_jsonb", "array_to_json", "row_to_json",
        # Функции информации об объектах
        "pg_get_expr", "pg_get_functiondef", "pg_get_function_arguments",
        "pg_get_function_identity_arguments", "pg_get_function_result",
        "pg_get_catalog_foreign_keys", "pg_get_constraintdef", "pg_get_userbyid",
        "pg_get_keywords", "pg_get_partkeydef",
        # Функции кодирования
        "pg_basetype", "pg_client_encoding", "pg_encoding_to_char", "pg_char_to_encoding",
        # Функции проверки валидности
        "pg_input_is_valid", "pg_input_error_info",
        # Функции информации об объектах
        "pg_get_serial_sequence", "pg_get_viewdef", "pg_get_ruledef",
        "pg_get_triggerdef", "pg_get_statisticsobjdef",
        # Функции информации о типах
        "pg_typeof", "format_type", "to_regtype", "to_regtypemod",
        ...
        "pg_collation_is_visible", "pg_conversion_is_visible", "pg_function_is_visible",
        "pg_opclass_is_visible", "pg_operator_is_visible", "pg_opfamily_is_visible",
        "pg_statistics_obj_is_visible", "pg_table_is_visible", "pg_ts_config_is_visible",
        ...
        # Функции приведения типов
        "cast", "text", "bool", "int2", "int4", "int8", "float4", "float8", "numeric",
        "date", "time", "timetz", "timestamp", "timestamptz", "interval",
        # ACL-функции
        "acldefault", "aclexplode", "makeaclitem",
        # Функции системной информации
        "pg_tablespace_location", "pg_tablespace_databases", "pg_settings_get_flags",
        "pg_options_to_table",
        # Функции транзакций/WAL
        "pg_current_xact_id", "pg_current_snapshot", "pg_snapshot_xip",
        ...
        # Функции уведомлений
        "pg_listening_channels", "pg_notification_queue_usage",
        # Функции состояния сервера
        "pg_postmaster_start_time",
        # Функции восстановления
        "pg_is_in_recovery",
        # Функции HypoPG
        "hypopg_create_index", "hypopg_reset", "hypopg_relation_size",
        "hypopg_list_indexes", "hypopg_get_indexdef", "hypopg_hide_index",
        "hypopg_unhide_index",
        # XML-функции
        "xml", "xmlcomment", "xmlconcat", "xmlelement", "xmlforest", "xmlpi",
        "xmlroot", "xmlexists", "xml_is_well_formed", "xml_is_well_formed_document",
        "xml_is_well_formed_content", "xpath", "xpath_exists", "xmltable",
        "xmlnamespaces",
        # Функции работы с сетью/IP
        "abbrev", "broadcast", "cidr_split", "family", "host", "hostmask",
        "inet_merge", "inet_same_family", "macaddr8_set7bit", "masklen", "netmask",
        "network", "set_bit", "set_masklen",
        # Функции полнотекстового поиска
        "array_to_tsvector", "get_current_ts_config", "numnode", "plainto_tsquery",
        "phraseto_tsquery", "querytree", "setweight", "strip", "to_tsquery",
        "to_tsvector", "ts_debug", "ts_delete", "ts_filter", "ts_headline",
        "ts_lexize", "ts_parse", "ts_rank", "ts_rank_cd", "ts_rewrite", "ts_stat",
        "ts_token_type", "tsvector_to_array", "websearch_to_tsquery",
        # Функции диапазонов
        ...
        "currval", "lastval",
        # Функции pg_trgm
        "similarity", "show_trgm", "show_limit", "word_similarity",
        "strict_word_similarity",
        # Функции pg_crypto
        "digest", "hmac", "encrypt", "gen_salt", "crypt",
        # Функции earthdistance/cube
        "earth_distance", "earth_box", "ll_to_earth", "earth_to_ll", "cube_distance",
        # Функции fuzzystrmatch
        "soundex", "difference", "levenshtein", "metaphone", "dmetaphone",
        # Оконные функции
    ...
    ALLOWED_NODE_TYPES: ClassVar[Set[Type[Node]]] = ALLOWED_STMT_TYPES | {
        ResTarget, ColumnRef, A_Star, A_Const, A_Expr, BoolExpr, BooleanTest,
        NullTest, RangeVar, JoinExpr, FromExpr, WithClause, CommonTableExpr,
        SubLink, MinMaxExpr, RowExpr, DefElem, SortBy, SortGroupClause, Integer,
        Float, String, BitString, Boolean, RawStmt, ParamRef, SQLValueFunction,
        FuncCall, TypeCast, TypeName, Alias, CaseExpr, CaseWhen, RangeSubselect,
        CoalesceExpr, NamedArgExpr, RangeFunction, A_ArrayExpr, WindowFunc,
        WindowDef, WindowClause, TableFunc, RangeTableFunc, RangeTableFuncCol,
        A_Indirection, A_Indices, GroupingSet, GroupingFunc, RangeTableSample,
        TableSampleClause, RowCompareExpr, CollateClause, TargetEntry,
        ScalarArrayOpExpr, NotifyStmt
    }

    # Разрешенные расширения PostgreSQL
    ALLOWED_EXTENSIONS: ClassVar[Set[str]] = {
        "hypopg", "pg_stat_statements", "pg_trgm", "btree_gin", "btree_gist",
        "earthdistance", "cube", "fuzzystrmatch", "intarray", "pgcrypto", "hstore",
        "ltree", "xml2", "tablefunc", "tsm_system_rows", "tsm_system_time",
        "unaccent", "uuid-ossp", "adminpack", "amcheck", "bloom", "citext",
        "dict_int", "dict_xsyn", "file_fdw", "intagg", "isn", "lo",
        "pg_buffercache", "pg_freespacemap", "pg_prewarm", "pg_visibility",
        "pgrowlocks", "pgstattuple", "plpgsql", "seg", "spi", "sslinfo",
        "postgres_fdw", "dblink", "mysql_fdw", "mongo_fdw", "tds_fdw",
        "oracle_fdw", "postgis", "pgrouting", "timescaledb", "pg_partman",
        "orafce", "pgaudit", "pgtap", "pgsphere", "pg_qualstats",
        "pg_hint_plan", "auto_explain", "pg_wait_sampling", "plv8",
        "pg_stat_monitor", "pg_cron", "pglogical", "pgq", "pgpool_adm",
        "pg_fuzzystrmatch", "pg_bigm", "pgvector", "rum", "zhparser",
        "ip4r", "chkpass", "tsvector2", "pg_stat_kcache", "wal2json",
        "pg_repack", "plperl", "plperlu", "plpython3u", "plpython", "pltcl",
        "pltclu", "pljava", "plrust", "aws_commons", "aws_s3", "h3",
        "pg_graphql", "pg_net", "pgjwt", "moddatetime", "age", "semver",
        "rdkit", "vector", "citus", "pg_tle", "pg_roaringbitmap", "pg_ivm",
        "pg_rational", "pg_partman_bgw", "q3c", "pg_track_settings",
        "pg_variables", "pg_walinspect", "pgmq", "address_standardizer",
        "address_standardizer_data_us", "postgis_raster", "postgis_sfcgal",
            ...
            for stmt in parsed:
                if isinstance(stmt, RawStmt):
                    if not isinstance(stmt.stmt, tuple(self.ALLOWED_STMT_TYPES)):
                        raise ValueError(
                            f"Разрешены только SELECT, ANALYZE, VACUUM, EXPLAIN, SHOW и другие read-only запросы. Получено: {stmt.stmt}"
                        )
                else:
                    if not isinstance(stmt, tuple(self.ALLOWED_STMT_TYPES)):
                        raise ValueError(
                            f"Разрешены только SELECT, ANALYZE, VACUUM, EXPLAIN, SHOW и другие read-only запросы. Получено: {stmt}"
                        )
    ...
    async def execute_query(
        ...
        if self.timeout:
            try:
                ...
                raise ValueError(
                    f"Выполнение запроса превысило таймаут {self.timeout} секунд в ограниченном режиме. "
                    "Упростите запрос или увеличьте таймаут."
                ) from e
            except Exception as e:
    ...
    def param_sql_to_query(query: str, params: List[Any]) -> str:
        ...
        """
        sql_params: List[Union[Composable, Literal]] = [
            p if isinstance(p, Composable) else Literal(p) for p in params
        ]
