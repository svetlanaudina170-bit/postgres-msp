...
async def test_top_queries_pg12_total_sort(mock_pg12_driver, mock_extension_installed):
    ...
    assert "Top 3 slowest queries by total execution time" in result or "Топ 3 самых медленных запросов по общему времени выполнения" in result
...
async def test_extension_not_installed(mock_pg13_driver, mock_extension_not_installed):
    ...
    assert "extension is required to report" in result or "требуется расширение pg_stat_statements" in result
...
async def test_error_handling(mock_pg13_driver, mock_extension_installed):
    ...
    assert "Error getting slow queries: Database error" in result or "Ошибка при получении медленных запросов: Database error" in result
