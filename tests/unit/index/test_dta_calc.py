...
async def test_index_exists(create_dta):
    ...
    with patch("pglast.parser.parse_sql", side_effect=Exception("Parsing error")):
        ...
        with pytest.raises(Exception, match="Error in robust index comparison|Ошибка при сравнении индексов"):
            dta._index_exists(
                index,
...
def test_explain_plan_diff():
    ...
    assert "PLAN CHANGES:" in diff_output or "ИЗМЕНЕНИЯ ПЛАНА:" in diff_output
    assert "Cost:" in diff_output or "Стоимость:" in diff_output
    assert "improvement" in diff_output or "улучшение" in diff_output

    # Verify it detected the change from Seq Scan to Index Scan
    assert "Seq Scan" in diff_output or "Seq Scan" in diff_output
    ...
    assert "Cannot generate diff" in empty_diff or "Невозможно создать сравнение" in empty_diff

    # Test with missing Plan field
    invalid_diff = ExplainPlanArtifact.create_plan_diff({"NotAPlan": {}}, {"NotAPlan": {}})
    assert "Cannot generate diff" in invalid_diff or "Невозможно создать сравнение" in invalid_diff
