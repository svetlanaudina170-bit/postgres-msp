...
async def test_force_readonly_enforcement():
    ...
    mock_conn_pool.pool_connect = AsyncMock()
    ...
    with patch("postgres_mcp.server.db_connection", mock_conn_pool), patch.object(SqlDriver, "_execute_with_connection", mock_execute):
        ...
        # Check that force_readonly is False by default (4th positional arg)
        ...
        assert mock_execute.call_args[0][3] is True
    ...
    with patch("postgres_mcp.server.db_connection", mock_conn_pool), patch.object(SqlDriver, "_execute_with_connection", mock_execute):
        ...
        await driver.execute_query("SELECT 1")
        assert mock_execute.call_count == 1
        # Check that force_readonly is always True (4th positional arg)
        assert mock_execute.call_args[0][3] is True
