...
async def test_update_statement(safe_driver):
    ...
    with pytest.raises(
        ValueError,
        match="валидации запроса|Error validating query",
    ):
        await safe_driver.execute_query(query)
...
async def test_select_into(safe_driver):
    ...
    FROM users
    """
    with pytest.raises(ValueError, match="валидации запроса|Error validating query"):
        await safe_driver.execute_query(query)
...
async def test_invalid_sql_syntax(safe_driver):
    ...
    with pytest.raises(ValueError, match="Не удалось разобрать SQL-запрос|Failed to parse SQL statement"):
